...
- 2026-02-14: `scripts/ralph.sh` now loads `.ralph/config.json` defaults (with env/arg overrides), writes structured `.ralph/iterations.jsonl` per iteration, manages `.ralph/ralph.pid`, handles `.ralph/pause` + `.ralph/inject.md` between iterations, and sends graceful stop notifications on SIGTERM/SIGINT.
- 2026-02-14: Production reverse-proxy template now lives at `scripts/nginx/ralph.xian.technology.conf`, including HTTPS redirect, websocket upgrade handling for `/api/ws`, and proxying all dashboard traffic to `127.0.0.1:8420`.
