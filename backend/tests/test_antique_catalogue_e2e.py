...
async def test_antique_catalogue_fixture_end_to_end(
    ...
    project_id = project_id_from_path(expected_project_path)
    ...
    iterations_response = await get_iterations(
        project_id, status_filter="all", limit=50, offset=0
    )
    assert iterations_response.total == 2
    assert [item.number for item in iterations_response.iterations] == [1, 2]

    first_iteration = await get_iteration_detail(project_id, 1)
    ...
    plan = await get_plan(project_id)
    ...
    report = await get_project_report(project_id)
